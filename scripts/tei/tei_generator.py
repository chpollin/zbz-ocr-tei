...
# ---------------------------------------------------------------------------
# OCR-Text laden (llm_corrected_c bevorzugt, Fallback mistral_results)
# ---------------------------------------------------------------------------

def load_ocr_text(doc_id: str, page: int) -> str | None:
    """Laedt OCR-Text: erst llm_corrected_c, dann mistral_results."""
    for base_dir in [LLM_CORRECTED_C_DIR, MISTRAL_RESULTS_DIR]:
...
def generate_tei_page(
    ...
    lang = "und"
    if metadata:
        lang_raw = metadata.get("lang", "und")
    ...
    lines.append(f'        <title type="main">{xml_escape(doc_id)}</title>')
    lines.append("      </titleStmt>")
...
def main():
    ...
    if args.doc and args.page:
        # Einzelne Seite
        metadata = get_test_metadata(args.doc)
