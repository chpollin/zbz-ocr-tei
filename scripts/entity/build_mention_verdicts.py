...
Exit codes: 0 clean, 1 validation failure (nothing written), 2 written but the wave
...
# The adjudicated result of the 2026-08-12 wave. A deviation is reported, never repaired.
...
def snapshot_label(manifest: dict) -> str:
    ...
    return max(stamps)[:10] if stamps else SNAPSHOT
...
def text_digests(docs, tei_dir: Path) -> dict[str, str]:
...
def load_inputs(*, sample_dir: Path = SAMPLE_DIR, entities_path: Path = ENTITIES_PATH,
                scan_path: Path = SCAN_PATH, tei_dir: Path = TEI_FINAL_DIR) -> dict:
    """Everything the store is built from, read once."""
...
def assemble(inputs: dict) -> dict:
    """The store payload, in the documented key order."""
