---
title: "Project Report zbz-ocr-tei: LLM-supported OCR and TEI Pipeline for the Digital Edition of Jeanne Hersch's Writings"
project:
  name: zbz-ocr-tei
...
* v3, 21.08.2026; v2, 09.07.2026; v1, 27.05.2026
* AI-Unterstützung: Claude Opus 4.7, Opus 4.8, Fable 5, Claude Code
* Berichtszeitraum: 29.01.2026 bis 21.08.2026; Adressat: Zentralbibliothek Zürich (ZBZ)
...
Die Ausgangslage bildet die Lieferung der ZBZ, die PDF-Scans der Drucktexte, das Masterfile mit den zugehörigen Metadaten, die zugehörigen PAGE-XML-Exporte, die Editionsrichtlinien und die 25 manuell erstellten Referenz-TEIs (Transkription in Transkribus, TEI-Auszeichnung in Oxygen; siehe Abschnitt 3). Letztere sind selektive Teiltranskriptionen und dienen als Ground Truth (siehe 6.1), was jede spätere Zahleninterpretation prägt. Der katalogische Gesamtbestand und seine bibliothekarische Erschließung bleiben bei der ZBZ.
...
Der Ordner `knowledge/` ist die Wissensbasis des Projekts nach der Promptotyping-Konvention, eine in Claude Code erzeugte und kuratierte Sammlung von Markdown-Dokumenten, die das Projektwissen abbildet und über den Projektverlauf iterativ gewachsen ist. Seit dem 21.08.2026 besteht sie aus zehn Dokumenten, die je eine oder mehrere Funktionen tragen. `index.md` führt die Navigation, dann folgen `project.md` (Auftrag, Material, Integration mit ZBZ, Transkribus und dem Annotationswerkzeug teiCrafter), `specification.md` (Anforderungen, Regelkatalog, Gates), `tei-mapping.md` (Auszeichnungsregelwerk und Entitätsmodell), `pipeline.md` (Architektur und Betrieb), `workflow.md` (Datenfluss, Viewer, Design), `methodology.md` (Arbeitsweise, CER-Messmethode, Governance), `verification.md` (Qualitätssicherung, Verifikation der Kernaussagen dieses Berichts und als Anhang die datierten Prüfprotokolle), `decisions.md` (durchnummeriertes Entscheidungsregister samt Plan) und `journal.md` (chronologisches Arbeitstagebuch als beschreibende Schicht neben der Git-Historie).

Der Ordner `data/` enthält Eingangs- und Referenzdaten und trennt Geliefertes von projektseitig Erzeugtem. Unter `data/source/` liegen die von der ZBZ gelieferten Startdaten, also die PDF-Scans, die manuell erstellten Referenz-TEIs samt zugehörigen PAGE-XML-Exporten, das Masterfile und die Editionsrichtlinien, dazu die versionierte Juni-Lieferung `zbz-lieferung-2026-06-21/` mit der aktuellen, vollständigen Fassung der Editionsrichtlinien, der zugehörigen ZBZ-Prüfvorlage (RelaxNG) und einem Provenienz-Dokument. Daneben stehen projektseitig erstellte Referenzdaten, das TEI-Schema `data/schema/zbz_hersch.rng`, das der ZBZ-Prüfvorlage samt den projektseitig ergänzten Header-Elementen entspricht (Entscheidung E68 im Register), der Ordner `data/curated_tei/`, der für handverifizierte TEIs reserviert ist, und unter `data/entities/` die kuratierte Entitätenliste mit ihrem Normdaten-Cache und den adjudizierten Urteilen (Abschnitt 4). Die per LLM erzeugte Dokumentklassifikation liegt in `doc_metadata.json`.

Der Ordner `output/` enthält alle generierten Datenströme (OCR, Layout, PAGE-XML, TEI, Entitätsvorschau) und ist bewusst nicht versioniert. Der Ordner `scripts/` enthält die von Claude Code generierte Python-Pipeline, nach Domäne in Unterpakete gegliedert (`ocr`, `layout`, `tei`, `eval`, `edition`, `entity`, `core`); die einzelnen Skripte sind in Anhang A aufgeführt. Die eigenständigen Prüfberichte, die neben der laufenden Auswertung entstanden sind, darunter eine unabhängige Gegenprobe der CER-Messung ohne Rückgriff auf den Projektcode (siehe 6.4), stehen als datierter Anhang in `knowledge/verification.md`. Die automatischen Tests liegen unter `tests/`. Sie sichern ab, dass die Werkzeuge bei jeder Änderung korrekt weiterarbeiten, etwa dass die CER-Statistik richtig rechnet, dass jedes ausgelieferte TEI gegen das Schema gültig bleibt und dass alle Skripte lauffähig sind. Die vollständige Test-Suite läuft auf dem Arbeitsrechner, wo auch die Daten liegen. Auf GitHub wird derselbe Testbestand bei jeder Änderung automatisch ausgeführt; die Tests, die die lokalen Daten brauchen, überspringen sich dort selbst, und der verbleibende, datenunabhängige Teil dient als Sicherheitsnetz gegen Fehler im Code.
...
* Texterkennung. Der ausgelieferte Bestand wurde mit Mistral Document AI[^7] über Azure AI Foundry erkannt. Das Modell erfasst neben Fließtext auch Tabellen und Listen und liefert seitenweises Markdown; weil die Schnittstelle je Anfrage nur eine begrenzte Seitenzahl annimmt, zerlegt die Pipeline große Dokumente automatisch in Teilstücke und fügt die Ergebnisse wieder zusammen. Dieser Zugang steht inzwischen nicht mehr zur Verfügung, der Endpunkt weist Anfragen ab. Für neue Läufe löst die Pipeline die Engine-Auswahl deshalb standardmäßig auf ein multimodales Gemini-Modell auf, das dieselbe Aufgabe übernimmt und in dieselben Ausgabepfade schreibt, sodass sich für die Folgestufen nichts ändert; die Mistral-Engine bleibt wählbar und dokumentiert, womit der ausgelieferte Bestand entstanden ist. Eine optionale, sprachmodellgestützte Nachkorrektur ist vorhanden, aber standardmäßig abgeschaltet, weil sie in der Erprobung bei bereits guter Erkennungsqualität die Texte messbar verschlechterte; wo eine geprüfte korrigierte Fassung vorliegt, wird sie der Rohfassung vorgezogen.
* Layoutanalyse. Die Strukturerkennung verbindet Docling[^8] mit einem nachgeschalteten Gemini-Schritt. Docling liefert die Basisstruktur, also Regionen mit Position und Typ, Gemini prüft die Labels und ergänzt fehlende Regionen. Im automatischen Modus wird je Seite gemessen, wie viel der Seitenfläche die erkannten Regionen abdecken. Bleibt die Abdeckung zu gering, erkennt Gemini das Layout direkt vom Scan vollständig neu und ersetzt das Docling-Ergebnis. Docling-Fassung und Gemini-Fassung bleiben als zwei getrennte Dateien erhalten, sodass nachvollziehbar bleibt, welche Erkennung von welcher Engine stammt.

### Austauschformate

Aus dem Layout- und dem OCR-Datenstrom werden zusätzlich PAGE-XML[^9] und ein METS-Manifest[^10] erzeugt, Austauschformate für externe Bearbeitungs- und Archivsysteme wie Transkribus. Sie entstehen aus denselben Quellen wie das TEI und stehen neben ihm; das TEI wird unmittelbar aus Layout und Text gebildet. Für den Rückweg nach Transkribus bündelt ein Exportskript die PAGE-XML-Dateien je Dokument, ein zweites lädt die Bündel über die REST-Schnittstelle hoch.
...
- [Entitäten](https://chpollin.github.io/zbz-ocr-tei/entities.html) als Übersicht der Entitätsvorschau je Dokument
...
Die CER ist der Anteil der Zeichen im Referenztext, die im erzeugten Text abweichen, definiert als Levenshtein-Distanz zwischen Referenz und Hypothese, geteilt durch die Zeichenzahl der Referenz. Die Levenshtein-Distanz ist die minimale Anzahl an Einzelzeichen-Operationen (Einfügung, Löschung, Ersetzung), um die eine Fassung in die andere zu überführen;[^12] implementiert ist sie über `rapidfuzz.distance.Levenshtein`. Aggregationseinheit ist das Dokument. Ein Perzentil-Bootstrap auf Dokumentebene (n = 25, B = 10 000, Seed 42) liefert Mittelwert, Median und deren 95-%-Vertrauensbereiche. Zur Einordnung dient die Transkribus-Konvention, nach der unter 2 % als publikationsreif, 2 bis 5 % als forschungstauglich und 5 bis 10 % als brauchbar für Volltextsuche gilt.[^12] Eine hohe CER bedeutet dabei nicht zwingend schlechte Zeichenerkennung; sie kann ebenso aus fehlerhafter Lesereihenfolge bei komplexem Layout folgen[^12] oder daraus, dass Mistral Document AI ein generelles Modell ohne Spezialisierung auf historische Schrift ist. Die Berechnung selbst ist ein einzelner Funktionsaufruf;[^13] die methodische Substanz liegt in der Aufbereitung der beiden Texte und in der Wahl der Referenz.
...
| E8 | `<pb/>` wird zu einem Leerzeichen, das die Whitespace-Normalisierung mit Nachbar-Whitespace zusammenzieht | Seitengrenze ist eine Wortgrenze |
...
| N6 / N7 | Backtick, Akut zu ASCII `'` | Backtick (U+0060), ´ (U+00B4) |
...
Bewusst nicht normalisiert werden Groß- und Kleinschreibung, Diakritika, Satzzeichen, die Unterscheidung von ß und ss sowie Zahlen, da dies substantielle Differenzen sind. Der case-sensitive Default folgt der Werkzeugpraxis von dinglehopper[^14] und jiwer, die Lowercasing als Opt-in führen; eine optionale case-insensitive Sekundärmetrik existiert (`casefold=True`). Die Erhaltung von Akzenten prüft eine eigene Metrik (HCPR) getrennt.[^15] Die inzwischen erfolgte Apostroph-Normalisierung des ausgelieferten Korpus (6.4) ist für die CER neutral, weil N16 bis N19 beide Apostrophformen auf dasselbe ASCII-Zeichen abbilden.

#### Verifikation der Messmethodik

Diese Verifikation betrifft die Korrektheit der CER-Messung selbst und ist von der TEI-Schemavalidierung (Abschnitt 4) zu unterscheiden. Sie ruht auf vier Schichten. Erstens handgerechnete Regressionstests (`tests/test_cer_extraction.py`), die unabhängig vom Korpus-Ergebnis die kanonische Formel, Case-Sensitivität, das Unterbleiben von Trimming, die `<choice>`-Auflösung, die Extraktions- und Normalisierungsregeln und die Fidelity/Scope-Zerlegung samt zeichengenauer Summenkontrolle festschreiben. Zweitens die Vereinheitlichung der zuvor drei separaten CER-Implementierungen (`benchmark_cer`, `cer_statistics_full`, `tei_validator --compare-ref`) auf gemeinsame kanonische Funktionen, sodass alle drei Pfade für dasselbe Dokument dieselbe Zahl liefern. Drittens der Abgleich der Konventionen mit externen Standards, dem Nenner als Referenzlänge (Transkribus), der NFC-Normalisierung (OCR-D),[^16] dem case-sensitiven Default (jiwer; OCR-D führt Case-Ignoranz nur als eigene Letter-Accuracy-Metrik), dem Volltextvergleich ohne Alignment-Trimming (dinglehopper)[^14] und dem paired Bootstrap für Deltas (Du 2025).[^17] Viertens eine unabhängige Gegenprobe, die alle publizierten Werte ohne Repository-Code reproduziert hat (6.4). Die Vergleichbarkeit von CER-Werten zwischen Werkzeugen bleibt trotzdem begrenzt, unter anderem weil bereits die Umwandlung strukturierter Ground Truth in Vergleichstext zur Fehlerquelle wird; die dokumentierten Extraktions- und Normalisierungsregeln sind die projektinterne Festlegung dieser Transformation.

#### Quantitative Schemavalidierung

Alle 285 finalen TEI validieren gegen `zbz_hersch.rng`, mit null blockierenden Verstößen gegen die Projektregeln R1 bis R7; die Schema- und Header-Prüfungen laufen zusätzlich als pytest-Gates bei jedem Push. Die informative Warnungsbilanz nach den Bestandsläufen (6.4) umfasst 2 017 Vorkommen in 256 Dokumenten und wird von zwei bewussten Kurationssignalen getragen, den leeren Sprecher-Slots der Interview-Dokumente (W17, 830 Vorkommen in 15 Dokumenten) und der Lesereihenfolge-Worklist (W19, 826 Seiten in 214 Dokumenten). Beide markieren Arbeitsvorrat; die Schemagültigkeit berührt keines von beiden. Die übrigen Klassen sind kleinere Konformitäts-Worklists zur div- und figure-Konvention (W15 mit 73, W16 mit 144 Vorkommen) sowie Einzelbefunde im ein- bis niedrigen zweistelligen Bereich.
...
| 1520 | C | FR | 2,25 | Zusatzseiten; Fußnoten-Demotion angewandt |
...
- Die semantische Anreicherung wurde nach einer frühen Erprobung mit sprachmodellgestützter Named Entity Recognition bewusst aus der Pipeline entfernt; das gelieferte Korpus ist entitätsfrei. An ihre Stelle ist die in Abschnitt 4 beschriebene deterministische Entitätsschicht getreten, die als Vorschau vorliegt und am Faksimile gemessen ist. Vor einer Übernahme in den Bestand stehen ein Schema-Härtungsschritt, ein Korpus-Trockenlauf und die Entscheidung mit der ZBZ, ob die Provenienz-Attribute der Markierungen in das ausgelieferte TEI übergehen; vier Konventionsfragen (Werke in der ersten Stufe, Kurationskanal für unsichere Kandidaten, Bindestrich-Komposita, Bildunterschriften) sind im Entscheidungsregister offen geführt. Die Identifikator-Zuordnung bleibt dabei ausschließlich deterministisch über Normdaten-Abfragen.
...
- Der Qualitäts-Proxy auf Basis der Dictionary Hit Rate generalisiert statistisch nicht (LOOCV-R² unter null) und wird deshalb ausschließlich als Plausibilitätsschranke geführt.[^20]
...
- Der produktive OCR-Zugang über Mistral Document AI ist nicht mehr erreichbar; Neu-Läufe laufen über die Gemini-Engine (Abschnitt 4). Eine Messung der Engine-Differenz auf den Referenzdokumenten steht aus und ist im Plan als offener Punkt geführt.

# Anhang A: Skripte der Pipeline

Aufruf als Modul (`python -m scripts.<paket>.<modul>`); das gepflegte Inventar steht in `scripts/README.md`, die vollständige Kommandoreferenz in `CLAUDE.md` des Repositorys.

## Geteilte Basis

- `config.py`: Pfade, Modellnamen, Konstanten.
- `utils.py`: dateiübergreifende Hilfsfunktionen.
- `core/loaders.py`: OCR-Datenstrom-Vorrang, Layout-Fund, Seitenauswahl (Text und Layout).
- `core/gemini.py`: gemeinsamer Gemini-Client der LLM-Stufen.
- `core/pb_split.py`: byte-identische Segmentierung an Seitenumbrüchen (Basis für Mirror und Audits).
- `core/tei_xml_utils.py`: XML-Hilfen samt Lesereihenfolge-Permutation.
...
- `ocr/ocr_pipeline.py`: steuert die OCR (Engine-Auswahl Mistral oder Gemini; für neue Läufe löst die Standardauswahl auf Gemini auf).
- `ocr/gemini_ocr_correct.py`: Ersatz- und Korrektur-OCR mit Gemini (zwei Varianten).
- `ocr/llm_postprocess.py`: optionale OCR-Nachkorrektur (standardmäßig abgeschaltet).
...
- `layout/generate_layout_overlays.py`, `layout/overlay.py`: Regionen-Overlays zur Kontrolle (optional Docling gegen Gemini) und ihre Zeichenroutine.
...
- `tei/tei_generator.py`, `tei/tei_mapping_prompt.py`: geteilte Bausteine (Markdown-zu-TEI, Mapping-Tabelle und Genre-Regeln).

## Validierung und Audits

- `tei/tei_validator.py`: RelaxNG-Schema, Projektregeln, Warnungen, ZBZ-Konformität.
- `tei/zbz_conformity.py`: Inline-GND-Konformitätsregeln, die RelaxNG nicht ausdrückt.
- `eval/benchmark_cer.py`, `eval/cer_statistics_full.py`, `eval/evaluate_ocr.py`: CER-Messstrecke auf gemeinsamen Funktionen.
- `eval/cer_statistics.py`, `eval/cer_statistics_runner.py`: Statistik-Bibliothek (Perzentil-Bootstrap, paired Bootstrap, HCPR) und deren Dateneinleser.
...
- `eval/blank_text_audit.py`: halluzinierter Text auf Leerseiten (Manifest gegen Docling-Nullregionen).
- `eval/stability_pilot.py`: Run-zu-Run-Streuung der LLM-Stufe.
...
- `tei/tei_footnote_marker_strip.py`: entfernt redundante Fußnoten-Marker aus dem Notentext.
...
- `tei/tei_reading_order_fix.py`: Dry-Run-Instrument der W19-Worklist (Korpus-Reorder widerlegt).
- `tei/tei_cover_strip.py`: entfernt E-Periodica-Deckblätter, operator-gated.
- `tei/marker_common.py`: geteiltes Trockenlauf- und Backup-Gerüst der Marker-Läufe.

## Entitätsschicht (Vorschau)

- `entity/fetch_gnd_variants.py`: baut den GND-Varianten-Cache über lobid.
- `entity/entity_lexicon.py`, `entity/entity_matcher.py`: Lexikon aus Liste, Cache, Varianten-Review und Markierungsregeln; deterministischer Matcher mit Offsets, Stufen und Regelangabe.
...
- `entity/entity_eval_sample.py`, `entity/build_mention_verdicts.py`, `entity/entity_verdict_guard.py`: stratifizierte Evaluationsziehung, Urteilsspeicher, Regressionsschranke.
- `entity/entity_risk_ranking.py`: Rangfolge der Markierungen nach Falsch-Positiv-Risiko.
- `entity/generate_entity_preview_data.py`, `entity/generate_entity_overview.py`: Viewer-Mirror der Vorschau und Übersichtsdaten für `entities.html`.
...
Wörtlicher Stand: Commit `eca68d1a`, 09.07.2026; verbindliche Quelle ist die genannte Codestelle. B.1 hat den ausgelieferten Bestand geformt, B.2 war für den Bestand nicht aktiv (produktive OCR über Mistral) und ist für neue Läufe die Standardengine. Mit "(Auszug)" markierte Blöcke sind gekürzt.
...
- pub_form: book, bookSection, journalArticle, encyclopedia, brochure, interview, anthology, other
- layout_type: A (single-column), B (two-column), C (monograph), D (special: photos, mixed, historical prints)
- title / author / date: if clearly visible, else null (Jeanne Hersch may be author or subject)
- description: one sentence
- has_jstor_cover: true if first page is a JSTOR cover
- num_columns: 1 or 2

Report only what you can clearly determine. Use null for uncertain fields.
```

### Layout-QA (`layout/layout_qa_gemini.py`, Gemini, Overlay-Bild und JSON) (Auszug)
...
TASK 1 - FIX WRONG LABELS: page numbers/running headers -> _filter; headings/titles/bibl -> section_header; bottom notes -> footnote; artifacts -> _filter.

TASK 2 - ADD MISSING REGIONS: bbox as page % (0-100); page_header y 0-5, page_footer y 88-100. CRITICAL for multi-column: check the rightmost 30% for uncovered text.

PICTURE DETECTION: photos/illustrations/logos/charts -> picture; framed empty areas -> _filter; text in a frame stays text.

OUTPUT: return ALL regions; keep existing text and bbox exactly; change only label/zbz_tag; score 0-100 (deduct 10 per wrong/missing label).
```
...
Labels: section_header, text, footnote, caption, page_header, page_footer, picture, table, list_item.
...
### TEI-Verfeinerung (`tei/tei_step2.py` mit `tei/tei_mapping_prompt.py`, Gemini, Seitenbild, Gerüst und OCR) (Auszug)
...
Kern des Prompts ist die vollständige Mapping-Tabelle (sieben Sektionen: Dokumentstruktur, Zeilen, Inline-Formatierung, Sprachwechsel, Korrekturen, Redebeiträge, Auslassungen) plus zehn genrespezifische Regelblöcke. Vollständiger Wortlaut: `MAPPING_TABLE` und `GENRE_RULES` in `tei/tei_mapping_prompt.py`.

## B.2 Weitere Prompts (für den ausgelieferten Bestand nicht aktiv)

### Gemini-Vision-OCR (`ocr/ocr_pipeline.py`, Standardengine für neue Läufe)
...
Stufe 1 (Analyse): "You are an OCR error detection specialist ..." Flagge nur echte OCR-Fehler (Zeichen, fehlende Akzente, rn zu m, cl zu d) und Plattform-Artefakte (JSTOR, e-periodica) als `ocr_artifact`. `original` muss exakter Substring sein; höchstens 50 Korrekturen je Seite; keine gültigen Schreibvarianten flaggen. Variante B verifiziert jede Korrektur zusätzlich gegen das Scan-Bild. Vier Few-Shot-Beispiele im Code.

Stufe 2 (Anwendung): "You are a precise text editor. Apply ONLY the listed corrections ..." Nur high/medium anwenden, low überspringen; `ocr_artifact` löschen; sonst zeichenidentisch; Markdown und Absätze erhalten; nur den korrigierten Text ausgeben.

Vollständiger Wortlaut: `build_analysis_prompt` und `build_correction_prompt` in `ocr/gemini_ocr_correct.py`.

### OCR-Nachkorrektur (`ocr/llm_postprocess.py`, Anthropic, drei Erprobungsvarianten)

Experiment mit drei System-Prompt-Varianten (A Baseline mit Analyse- und Korrekturblock, B schlank nur korrigierter Text, C Few-Shot mit echten Mistral-Fehlerbeispielen). Standardmäßig abgeschaltet. Wortlaut: `_prompt_variant_a/b/c` in `ocr/llm_postprocess.py`.
...
[^3]: [https://dhcraft.org/Promptotyping](https://dhcraft.org/Promptotyping/); ein Videotutorial zu Promptotyping: [https://youtu.be/8sUe4Jkh3uQ](https://youtu.be/8sUe4Jkh3uQ).
...
[^15]: Levchenko, Maria. 2025. LM4DH 2025 Workshop at RANLP 2025, Varna, S. 75–85, DOI 10.26615/978-954-452-106-6-007; Preprint arXiv:2510.06743 (frequenzbasierte HCPR-Adaption zur Diakritika-Erhaltung; CER-Werte für russischen Druck des 18. Jahrhunderts).

[^16]: OCR-D. „Quality Assurance in OCR-D". [https://ocr-d.de/en/spec/ocrd_eval](https://ocr-d.de/en/spec/ocrd_eval) (CER-Definition, Unicode-Normalisierung NFC, Grapheme-Cluster, Letter Accuracy als eigene, hier nicht genutzte Metrik).

[^17]: Du, W. 2025. „When +1% Is Not Enough: A Paired Bootstrap Protocol for Evaluating Small Improvements". arXiv:2511.19794.

[^18]: Greif, Griesshaber, Greif 2025. arXiv:2504.00414 (deutschsprachige Adressbücher 1754–1870, überwiegend Fraktur; Transkribus Print M1 mit und ohne multimodale Gemini-Nachkorrektur, Gemini 2.0 Flash zero-shot, GPT-4o direkt).

[^19]: Kanerva und Ledins 2025. arXiv:2502.01205 (GPT-4o als LLM-as-judge für OCR-Qualität ohne Ground Truth, mehrsprachige historische Korpora).

[^20]: Nach dem Ansatz von Stroebel et al. 2022 (Dictionary-basierte OCR-Qualitätsschätzung); die projektinterne Prüfung ergab ein negatives LOOCV-R² des zusammengesetzten Schätzers.
