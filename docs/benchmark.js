...
(function () {
    ...
    function sum(arr, fn) {
        return arr.reduce((s, d) => s + fn(d), 0);
    }
    ...
    function init() {
        ...
        ZBZ.log('Benchmark', `${DATA.documents.length} Docs | Mistral vs DeepSeek`);
    }
    ...
    function renderMetrics() {
        ...
        const mPages = sum(d, (x) => x.mistral.pages);
        const dPages = sum(d, (x) => x.deepseek.pages);
        const avgSpp = (sum(d, (x) => x.mistral.spp) / d.length).toFixed(1);
        const totalTime = sum(d, (x) => x.mistral.time).toFixed(1);
    ...
    function renderDocs() {
        const container = ZBZ.$('#doc-list');

        DATA.documents.forEach((doc, idx) => {
            const mPct = (doc.mistral.chars / (doc.mistral.chars + doc.deepseek.chars) * 100).toFixed(1);
            const dPct = (100 - parseFloat(mPct)).toFixed(1);
            const sharedStr = doc.deepseek.available.join(', ');

            let pageBtns = '';
            for (let p = 1; p <= doc.totalPages; p++) {
                const both = doc.deepseek.available.indexOf(p) !== -1;
                const isStart = (p === doc.deepseek.available[0]);
    ...
    async function showPage(docId, page, btn) {
        if (btn) {
            const btns = btn.closest('.page-nav').querySelectorAll('.pg');
            btns.forEach((b) => { b.classList.remove('active'); });
            btn.classList.add('active');
        }

        const pp = ZBZ.padPage(page);
        ZBZ.$(`#img-el-${docId}`).src = `${ZBZ._basePath}images/${docId}/${docId}_p${pp}.png`;

        const mt = await fetchText(`${ZBZ._basePath}../output/mistral_results/${docId}_p${page}.md`);
        const mEl = ZBZ.$(`#mt-${docId}`);
        mEl.textContent = mt || '';
        if (!mt) mEl.innerHTML = '<div class="empty-state">Keine Daten</div>';

        const dt = await fetchText(`${ZBZ._basePath}../output/ocr_results/${docId}_p${page}.md`);
        const dEl = ZBZ.$(`#dt-${docId}`);
        dEl.textContent = dt || '';
    ...
    function initEvents() {
        ...
        document.addEventListener('click', (e) => {
            const btn = e.target.closest('.pg[data-doc]');
            if (!btn) return;
            showPage(btn.getAttribute('data-doc'), parseInt(btn.getAttribute('data-page')), btn);
        });

        document.addEventListener('click', (e) => {
            const img = e.target.closest('.preview-col-body img');
            if (!img) return;
