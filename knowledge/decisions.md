...
| O24 | `tei_validator --compare-ref` zeigt falschen Referenz-CER | `compute_cer` wird in `tei_validator` aus `evaluate_ocr` importiert, existiert dort aber nicht (die Funktion heisst `calculate_cer`, andere Skala: Ratio 0-1 statt Prozent). Import schlaegt seit jeher fehl, der Validator faellt still auf eine grobe Laengen-Approximation zurueck. Entdeckt beim scripts-Refactor 2026-05-27 | Quality/Validierung | eigener Fix (Skala pruefen, nicht nur Funktionsname tauschen) |
