...
| O23 | `tei_final`-Header nicht schema-valide (idno) | Alle 285 `output/tei_final/{doc}_final.xml` tragen `<idno type="docID">` im `<publicationStmt>`, was `zbz_hersch.rng` dort nicht erlaubt (4-9 Schema-Fehler je Doc). Der Default-Validator zeigt es nicht, weil er `tei_unified` prueft (dort sauber) — Drift `tei_final` vs `tei_unified`. Body bleibt valide; betrifft nur den Header der ausgelieferten Voll-TEI. Entdeckt bei E65. | Quality/Validierung | eigener Fix-Lauf, [quality.md](quality.md) |
