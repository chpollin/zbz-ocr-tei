...
| E32 | Unified TEI Pipeline (Rule-Based Scaffold + Gemini Refinement) | 4-Stufen-Pipeline: Step 1 (enhanced rule-based TEI mit lb, head, note, semantic div, interview-speaker), Step 2 (Gemini Refinement mit Mapping-Table-Prompt, 1 Call/Seite), Step 3 (Document Assembly mit teiHeader/facsimile/body + Post-Assembly-Fix), Step 4 (RelaxNG Validation). Post-Processing: `fix_gemini_tei()` mit 8 Fix-Stufen (ab-unwrap, head-in-speaker, head-p-flatten, head-after-content, epigraph-after-content, sp-mixed-split, inline-in-div-wrap, entity-reannotation) + `_fix_orphaned_body_children()` Post-Assembly. Production Run: 286 Docs, 22/23 VALID nach ersten 23 Docs. Kosten Step 2: ~$17 | 2026-03-07 | [PIPELINE](PIPELINE.md) |
