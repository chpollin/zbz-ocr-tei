...
Der Weg vom Masterfile zum verarbeiteten Korpus ist ein vierstufiger Trichter, den das Audit-Skript `corpus_audit` reproduzierbar aus den Primärquellen ableitet: 325 im Masterfile gelistete Texte, davon 289 digitalisiert, davon 286 als PDF geliefert (nicht geliefert: 1745, 1750, 1970), davon 285 mit finalem TEI (das gelieferte PDF ohne TEI ist Dokument 10, siehe 6.3).
...
Der Ordner `knowledge/` ist ein Promptotyping-Vault, eine an eine Obsidian-Forschungsvault angelehnte, in Claude Code erzeugte und kuratierte Wissensbasis in Markdown, die das Projektwissen abbildet und über den Projektverlauf iterativ wächst; einzelne Dokumente entstehen, wachsen oder werden zusammengeführt. Leitprinzip ist die Single Source of Truth. Jeder Fakt steht in genau einem Dokument, auf das die übrigen verweisen. Hervorzuheben sind das chronologische Arbeitstagebuch `journal.md` als beschreibende Schicht neben der Git-Historie und das durchnummerierte Entscheidungsregister `decisions.md`, das zum Berichtszeitpunkt 94 datierte Entscheidungen samt verworfenen Alternativen führt.
...
- Die Lesereihenfolge auf Doppelseiten und Mehrspaltern ist generatorseitig behoben (spalten- und bandbewusste Permutation, regressionsgetestet); die Umstellung des ausgelieferten Bestands ist als reversible Vorschau vorbereitet, die die betroffenen Seiten von 831 auf 39 senkt, und wartet auf die Freigabe. Die 39 Restseiten sind exakt die Fälle, in denen die automatische Korrektur unzuverlässig ist, und gehen in die Faksimile-Review.
- Die Run-zu-Run-Stabilität der nicht-deterministischen LLM-Stufen ist nicht quantifiziert; eine Pilotmessung (5 Dokumente mal 3 Läufe) ist freigegeben und steht aus.
...
- Auf der Datenseite bleiben extern zu klären die drei nicht gelieferten Texte (1745, 1750, 1970) und das gelieferte PDF ohne finales TEI (Dokument 10). Dazu kommen zwei registrierte Einzeldefekte, die Seite mit geleakter Modell-Absage infolge degenerierter Basis-OCR (Dokument 1520, Seite 70; die Einzelseiten-Reparatur per Re-OCR ist vorbereitet) und die ausstehende Adjudikation des Dokuments 30.
