...
Im ZBZ-Projekt wurde die Verifikationskaskade als operativer Prozess auf den gesamten Korpus angewandt: ein Agent-Based Quality Screening, bei dem Claude Code alle 285 TEI-Dokumente systematisch prüft. Der Prozess durchläuft sieben Schichten: Scan-Qualität, OCR-Treue, Layout-Korrektheit, TEI-Struktur (Schema-Validierung), Referenz-Vergleich (wo ZBZ-Referenz-TEI vorliegt), Entity-Plausibilität und Gesamtkohärenz. Pro Dokument entsteht ein maschinenlesbarer Befund (Review-JSON) mit Layer-Scores und konkreten Findings. Die 285 Dokumente wurden in vier Komplexitätstiers eingeteilt (1–3 Seiten, 4–8, 9–20, über 20) und in 58 Batches parallelisiert über etwa 40 Agent-Invocations verarbeitet.

Die Ergebnisse zeigen, wie die Kaskade in der Praxis funktioniert. Die automatische Verifikation (RelaxNG-Validierung) bestätigte 284 von 285 Dokumenten als schema-valide. Die kontextuelle Verifikation identifizierte zehn systematische Muster (P1–P10), darunter: Entity-Typ-Konflikte bei Personen, deren Namen auch Werktitel sind (Kierkegaard, Nietzsche), Gattungsbegriffe als falsche Entity-Tags ("Mensch", "Est", "Gott", "Rolle" — betrifft ca. 30% der Dokumente), und mehrspaltige Zeitungslayouts, die systematisch OCR-Halluzinationen erzeugen (Journal de Genève, Cooperation). Die visuelle Verifikation deckte auf, dass Gemini im TEI-Erzeugungsschritt nebenbei OCR-Fehler korrigiert (ein undokumentierter Qualitätsgewinn) und dass kurze Dokumente (Tier 1, 1–3 Seiten) deutlich fehleranfälliger sind als längere (Tier 2–4), weil sie häufiger aus komplexen Zeitungslayouts stammen.

Das Gesamtergebnis: 210 Dokumente (74%) sind publikationsreif, 43 (15%) tragen Hinweise für die menschliche Kuration, 32 (11%) erfordern Nacharbeit. Die 32 nacharbeitspflichtigen Dokumente verteilen sich auf drei Ursachen: OCR-Halluzinationen bei Zeitungslayouts (8), Entity-False-Positives bei Gattungsbegriffen (15) und Strukturprobleme (9). Der Screening-Status wird im TEI-Header als `<revisionDesc>` gespeichert und reist mit dem Dokument.

Die fachliche Verifikation steht noch aus: Die Editionswissenschaftlerin der ZBZ wird die als NEEDS_REVIEW markierten Dokumente im Curation Editor prüfen. Erst dieser Vergleich — was hat der Agent gefunden, was hat die Fachexpertin gefunden, was haben beide übersehen — liefert die empirische Basis für die Evaluation der Kaskade. Das Screening ist als Pre-Curation-Prozess positioniert: Es erzeugt vorgeprüfte Dokumente mit strukturiertem Befund, nicht fertige Editionen. Der Agent reduziert die Menge an Dokumenten, die fachliche Aufmerksamkeit erfordern, und fokussiert die menschliche Expertise auf die tatsächlich schwierigen Fälle.
...
Erste Daten aus dem ZBZ-Projekt: Von 285 verarbeiteten Dokumenten sind 284 schema-valide (99,6%). Von 285 verarbeiteten Dokumenten bewertete das Agent-Based Quality Screening 210 (74%) als publikationsreif, 43 (15%) mit Hinweisen fuer Kurator:innen und 32 (11%) als nacharbeitspflichtig. Die 32 nacharbeitspflichtigen Dokumente teilen sich in drei Hauptursachen: OCR-Halluzinationen bei mehrspaltigen Zeitungslayouts (8), Entity-False-Positives bei Gattungsbegriffen (15) und Strukturprobleme (9). Die Eskalationsrate sank ueber die vier Verarbeitungstiers (einfache Dokumente zuerst, komplexe zuletzt), was mit der These konsistent ist, dass die epistemische Infrastruktur ueber die Chargen waechst. Die entscheidende Messung folgt, wenn die Editionswissenschaftlerin der ZBZ die als NEEDS_REVIEW markierten Dokumente im Curation Editor prueft: Welche Probleme findet sie, die der Agent uebersehen hat?
